...
* [⇢ ⇢ Grafana Alloy: the unified collector](#grafana-alloy-the-unified-collector)
* [⇢ ⇢ Centralised logging with Loki](#centralised-logging-with-loki)
* [⇢ ⇢ ⇢ Alloy configuration for logs](#alloy-configuration-for-logs)
* [⇢ ⇢ ⇢ Querying logs with LogQL](#querying-logs-with-logql)
* [⇢ ⇢ Metrics with Prometheus](#metrics-with-prometheus)
* [⇢ ⇢ ⇢ Alloy configuration for application metrics](#alloy-configuration-for-application-metrics)
* [⇢ ⇢ ⇢ Kubernetes metrics: kubelet, cAdvisor, and kube-state-metrics](#kubernetes-metrics-kubelet-cadvisor-and-kube-state-metrics)
* [⇢ ⇢ ⇢ Infrastructure metrics: Kafka, Redis, MinIO](#infrastructure-metrics-kafka-redis-minio)
* [⇢ ⇢ Distributed tracing with Tempo](#distributed-tracing-with-tempo)
* [⇢ ⇢ ⇢ Understanding traces, spans, and the trace tree](#understanding-traces-spans-and-the-trace-tree)
* [⇢ ⇢ ⇢ How trace context propagates](#how-trace-context-propagates)
* [⇢ ⇢ ⇢ Implementation](#implementation)
* [⇢ ⇢ ⇢ Alloy configuration for traces](#alloy-configuration-for-traces)
* [⇢ ⇢ Async ingestion trace walkthrough](#async-ingestion-trace-walkthrough)
* [⇢ ⇢ ⇢ Step 1: Ingest a document](#step-1-ingest-a-document)
* [⇢ ⇢ ⇢ Step 2: Find the ingestion trace](#step-2-find-the-ingestion-trace)
* [⇢ ⇢ ⇢ Step 3: Fetch the complete trace](#step-3-fetch-the-complete-trace)
* [⇢ ⇢ ⇢ Step 4: Analyse the async trace](#step-4-analyse-the-async-trace)
* [⇢ ⇢ ⇢ Viewing traces in Grafana](#viewing-traces-in-grafana)
* [⇢ ⇢ End-to-end search trace walkthrough](#end-to-end-search-trace-walkthrough)
* [⇢ ⇢ ⇢ Step 1: Make a search request](#step-1-make-a-search-request)
* [⇢ ⇢ ⇢ Step 2: Query Tempo for the trace](#step-2-query-tempo-for-the-trace)
* [⇢ ⇢ ⇢ Step 3: Analyse the trace](#step-3-analyse-the-trace)
* [⇢ ⇢ ⇢ Step 4: Search traces with TraceQL](#step-4-search-traces-with-traceql)
* [⇢ ⇢ ⇢ Viewing the search trace in Grafana](#viewing-the-search-trace-in-grafana)
* [⇢ ⇢ Correlating the three signals](#correlating-the-three-signals)
* [⇢ ⇢ Grafana dashboards](#grafana-dashboards)
* [⇢ ⇢ Results: two days well spent](#results-two-days-well-spent)
* [⇢ ⇢ SLIs, SLOs and SLAs](#slis-slos-and-slas)
* [⇢ ⇢ Using Amp for AI-assisted development](#using-amp-for-ai-assisted-development)
* [⇢ ⇢ Other changes along the way](#other-changes-along-the-way)
...
[Full Alloy configuration with detailed metric filtering](https://github.com/florianbuetow/x-rag/blob/main/infra/k8s/monitoring/alloy-config.yaml)  
...
[Quick video explaining the difference between trace IDs and span IDs in OpenTelemetry](https://www.youtube.com/watch?v=KPGjqus5qFo)  
...
[![Async ingestion trace in Grafana Tempo](./x-rag-observability-hackathon/index-trace.png "Async ingestion trace in Grafana Tempo")](./x-rag-observability-hackathon/index-trace.png)  

[![Ingestion trace node graph showing service dependencies](./x-rag-observability-hackathon/index-node-graph.png "Ingestion trace node graph showing service dependencies")](./x-rag-observability-hackathon/index-node-graph.png)  
...
[![Search trace in Grafana Tempo](./x-rag-observability-hackathon/search-trace.png "Search trace in Grafana Tempo")](./x-rag-observability-hackathon/search-trace.png)  

[![Search trace node graph showing service flow](./x-rag-observability-hackathon/search-node-graph.png "Search trace node graph showing service flow")](./x-rag-observability-hackathon/search-node-graph.png)  
...
[![X-RAG Overview dashboard](./x-rag-observability-hackathon/dashboard-xrag-overview.png "X-RAG Overview dashboard")](./x-rag-observability-hackathon/dashboard-xrag-overview.png)  
[![Pod System Metrics dashboard](./x-rag-observability-hackathon/dashboard-pod-system-metrics.png "Pod System Metrics dashboard")](./x-rag-observability-hackathon/dashboard-pod-system-metrics.png)  
...
[Amp - AI coding agent by Sourcegraph](https://ampcode.com/)  
