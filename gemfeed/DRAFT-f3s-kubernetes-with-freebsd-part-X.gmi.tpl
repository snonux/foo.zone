...
│   ├── loki.yaml
│   ├── ...
│   └── (21 Application manifests total)
...
1. **argocd-apps/**: Application manifests that ArgoCD reads
...
Created `argocd-apps/miniflux.yaml`:
...
Currently, all Application manifests are applied manually with `kubectl apply -f argocd-apps/`. An alternative is the "app-of-apps" pattern:

Create a root Application that deploys all other Applications:
