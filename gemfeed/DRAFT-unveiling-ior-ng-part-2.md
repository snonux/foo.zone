...
The short answer: use Docker. One command, no toolchain setup, works from any Docker-capable Linux host with BTF available:
...
First run builds a Rocky Linux 9 builder image (~15–20 minutes). Subsequent runs reuse the cached image and finish in under a minute. The resulting static binary lands at `./ior`.

That's the officially supported install path, and it's the right one for anyone who just wants to run ior without living in its build system.
